...
<script>
	...
	import { geoPath, geoAlbersUsa, csv, scaleSequential, interpolateGreens } from 'd3';
	...
	const colorScale = scaleSequential(interpolateGreens).domain([0, 500]); // Adjust domain as needed
	...
	onMount(async () => {
		...
		// Load CSV data
		data = await csv('/assets/data/8_ban_and_group_counties.csv');
